            ...
            <div class="col-sm-6 col-md-4 col-lg-3 mb-4 product-card">
                <div class="card h-100 shadow animate-fadein">
                    <div class="position-relative">
                        <img src="{{ $producto->urlFoto 
                            ? (Storage::disk('public')->exists($producto->urlFoto) 
                                ...
                                : (filter_var($producto->urlFoto, FILTER_VALIDATE_URL) 
                                    ? $producto->urlFoto 
                                    : asset('img/generica.png')))
                            : asset('img/generica.png') }}"
                            class="card-img-top rounded-top img-fluid" alt="Error al cargar la imagen del producto"
                    ...
                    <div class="crud-actions d-flex justify-content-around py-2">
                        <a href="{{ route('productos.show', $producto->id) }}" class="btn btn-outline-primary" title="Ver">
                            <i class="fa fa-eye fa-xl"></i>
                        </a>

                        @auth
                            @if (Auth::user()->role === 'administrador')
                                <a href="{{ route('productos.edit', $producto->id) }}" class="btn btn-outline-success" title="Editar">
